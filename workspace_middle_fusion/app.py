...
def load_pipeline():
    try:
        ...
        unified_model.load_state_dict(torch.load(os.path.join(MODELS_DIR, "joint_fusion_model.pth"), weights_only=True, map_location=DEVICE))
        
        unified_model.to(DEVICE)
        unified_model.eval()
        return unified_model, scaler, tokenizer
...
unified_model, scaler, tokenizer = load_pipeline()
...
with col2:
    ...
    if st.button("Analyze Match Environment", type="primary"):
        ...
        if unified_model:
            ...
            x_num = np.array([[stats[k] for k in ordered_keys]])
            ...
            # Since Middle Fusion model just outputs win_prob directly combining both, toxicity isn't output.
            # We'll use mocked toxicity just for the dashboard display to keep the UI intact, or just say it's baked into win_prob now!
            toxicity = 0.0
            flags = ["Integrated into Win Probability"]
            fb = [f"The Unified Model predicts a {win_prob*100:.1f}% chance of winning, taking into account both your gameplay stats and the nature of your team communications seamlessly via Middle Fusion!"]
